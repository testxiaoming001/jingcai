...
class WalletService
{
        ...
        foreach ($money_field as $v) {
            ...
            if ($wallet[$v['field']] != $data[$v['field']]) {
                ...
                if ($is_send_message == 1) {
                    MessageService::MessageAdd($wallet['user_id'], '账户余额变动', $log_data['msg'], BaseService::$message_business_type, $wallet['id']);
                }
            }
        }
    ...
    public static function UserWalletMoneyUpdate($user_id, $money, $type, $field = 'normal_money', $business_type = 0, $msg_title = '钱包变更')
    {
        ...
        if ($wallet['code'] == 0) {
            ...
            $msg = ($log_data['operation_type'] == 1) ? '增加' : '减少';
            $log_data['msg'] = $msg_title . ' [ ' . self::$money_type_list[$log_data['money_type']]['name'] . '金额' . $msg . $log_data['operation_money'] . ' ]';
            if (!self::WalletLogInsert($log_data)) {
                Db::rollback();
            ...
            // 消息通知
            MessageService::MessageAdd($wallet['data']['user_id'], '钱包变更', $log_data['msg'], BaseService::$message_business_type, $wallet['data']['id']);
     ...
     */
    public static function sendUsdtWhenSpeak($data)
    {
    ...
    public static function wallectLogsByTgId($tgId)
    {
        $user = Db::name('User')->where(['user_telegram_id' => $tgId])->find();
        $logs = Db::name('PluginsWalletLog')->where(['user_id' => $user['id']])->order(['add_time' => 'desc'])->select();
        return $logs;
    }
     ...
     * @throws \think\exception\DbException
     */
    public static function sendCommission($map, $amount = 0.00)
    {
        ...
        $res = WalletService::UserWalletMoneyUpdate($user['id'], $amount, 1, 'normal_money');
        if ($res['code'] != 0) {
            //钱包状态异常
            throw  new \Exception($res['msg']);
        }
