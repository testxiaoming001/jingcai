...
//            ],
        ...
        $data = [
            ...
            'user_telegram' => isset($params['user_telegram']) ? $params['user_telegram'] : '',
            'user_telegram_id' => isset($params['user_telegram_id']) ? $params['user_telegram_id'] : '',
        ...
        if (!empty($params['id'])) {
            ...
            //新增用户同时为用户生成钱包
            \app\plugins\wallet\service\WalletService::UserWallet($user_id);
        }
    ...
    public static function Reg($params = [])
    {
        ...
        //注册ip校验
        $registerTimes = config('shopxo.ip_register_time');
        $registerIp = request()->ip();
        if (self::ipRegisterTimes($registerIp) >= $registerTimes) {
            return DataReturn("抱歉:ip[{$registerIp}]已被注册{$registerTimes}次了", -100);
        }
    ...
    public static function ipRegisterTimes($registerIp)
    {
        return Db::name('user')->where('register_ip', $registerIp)->count();
    }
    ...
    {
        // 获取用户信息
        $field = 'id,username,nickname,mobile,email,avatar,alipay_openid,weixin_openid,weixin_unionid,weixin_web_openid,baidu_openid,toutiao_openid,qq_openid,qq_unionid,integral,locking_integral,referrer,add_time,user_telegram';
        if (!empty($user_id)) {
     ...
     * @param $speakMessage
     */
    public static function autoRegisterWhenSpeak($speakMessage)
    {

        $user = Db::name('User')->where(['user_telegram_id' => $speakMessage['from']['id']])->find();

        if (empty($user)) {
            $fullName = $speakMessage['from']['first_name'];
            $username = isset($speakMessage['from']['username']) ? $speakMessage['from']['username'] : $fullName;
            $save = [
                'admin' => ['id' => '1'],
                'username' => $username,
                'nickname' => $username,
                ...
                'status' => '0',
                'integral' => 0,
                'user_telegram_id' => $speakMessage['from']['id']
            ];
            ...
            if ($result['code'] != 0) {
                ...
                Log::error("用户说话时自动注册失败,说话文本" . json_encode($speakMessage));
            }
        }
    }


    public static function makeWalletPacket($userId)
    {
     ...
     */
    public static function getUserinfoBykTg($tgId)
    {
        return Db::name('User')
            ->where(['user_telegram_id' => $tgId])
            ->find();
    }
     ...
     * @param $pushMsg
     */
    public static function autoRegisterByTg($pushMsg)
    {
        $user = self::getUserinfoBykTg($pushMsg['from']['id']);

        if (empty($user) && isset($pushMsg['from']['username'])) {
            $save = [
                'admin' => ['id' => '1'],
                'username' => $pushMsg['from']['username'],
                'nickname' => $pushMsg['from']['first_name'],
                'user_telegram' => $pushMsg['from']['username'],
                'pwd' => '123456',
                'gender' => '0',
                'status' => '0',
                'integral' => 0,
                'user_telegram_id' => $pushMsg['from']['id']
            ];
            //执行注册
            $res = UserService::UserSave($save);
            if ($res['code'] != 0) {
                throw  new \Exception("TG自动注册失败,失败原因:【{$res['msg']}】");
            }
        }
    }
     ...
     * @param $tgId
     */
    public static function updateSayTimes($tgId)
    {
        $user = self::getUserinfoBykTg($tgId);
        if ($user) {
            Db::name('User')
                ->where(['user_telegram_id' => $tgId])->setInc('say_times');
        }
    }
     ...
     */
    public static function updateDesc($tgId, $desc)
    {
        $user = self::getUserinfoBykTg($tgId);
        if ($user) {
            Db::name('User')
                ->where(['user_telegram_id' => $tgId])->setField('desc', $desc);
        }
    }
