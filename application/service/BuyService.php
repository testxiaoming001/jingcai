...
class BuyService
{
    ...
    public static function CartList($params = [])
    {
        ...
        $data = Db::name('Cart')->alias('c')
            ->leftJoin(['__GOODS__' => 'g'], 'g.id=c.goods_id')
            ->where($where)->field($field)->order('c.id desc')->select();
...
//            }
    ...
    public static function OrderInsert($params = [])
    {
        ...
        // 获取用户账户信息
        $user = Db::name('User')->field('id,pwd,pay_pwd,salt,username,mobile,email')->find($params['user']['id']);
        //支付安全密码校验
        if (LoginPwdEncryption($params['pay_pwd'], $user['salt']) != $user['pay_pwd']) {
            return DataReturn('支付密码错误', -4);
        }
