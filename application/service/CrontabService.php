...
class CrontabService
{
     ...
     * @param array $params
     */
    public static function pushTgBanner($params = [])
    {
        $prevPlayId = cache('prev_play_id');
        $where = [
            ['expire_time', '>', time()],
        ];
        if ($prevPlayId) {
            array_push($where, ['id', '>', $prevPlayId]);
        }
        $banner = Db::name('tg_banner')->where($where)->find();
        if (empty($banner)) {
            $banner = Db::name('tg_banner')->where([['expire_time', '>', time()]])->find();

        }

        cache('prev_play_id', $banner['id']);
        $tgService = new TelegramBotService();
        $tgService->sedDefaultGroupMessage($banner['contents']);
