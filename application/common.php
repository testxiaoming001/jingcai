...
* ===============================================================================
*/
function cut_html_str($str, $lenth=100, $replace='', $anchor='<!-- break -->'){
 ...
 * @param $userId
 */
function syncUserReliabilityToGoodsReliability($userId)
{

    $userReliability = \think\Db::name('user')->where(['id'=>$userId])->value('user_reliability');
    \think\Db::name('goods')->where([
         'uid'=>$userId
    ])->setField('goods_reliability',$userReliability);
}




function getPublishName($uid)
{
    $username = \think\Db::name('user')->where(['id'=>$uid])->value('username');
    return empty($username)?'admin':substr_cut($username);
}
?>
